...
#import "ForceAlertPasswordVC.h"

#define kTEXT_FONT [UIFont systemFontOfSize:13]
#define kHEAD_VIEW_H 680.0/1335*kSCREENHEIGHT
@interface ForceAlertPasswordVC ()
...
@implementation ForceAlertPasswordVC{
    CGFloat imgW;
    CGFloat imgH;
    UIImage *headerImg;
}
...
- (void)configularCell:(UITableViewCell*)cell indexPath:(NSIndexPath*)indexPath{
    ...
    if (1 == indexPath.section) {
        ...
        UILabel *leftLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, 80, 50)];
        leftLabel.text = self.leftNameArr[indexPath.row];
        leftLabel.font = kTEXT_FONT;
        [cell.contentView addSubview:leftLabel];
        
        UITextField *rightTextField = [[UITextField alloc] initWithFrame:CGRectMake(CGRectGetMaxX(leftLabel.frame)+10, 0, kSCREENWIDTH-(CGRectGetMaxX(leftLabel.frame)+10)-60, 50)];
        ...
        if (0 == indexPath.row) {
            ...
            rightSwitch.frame = CGRectMake(kSCREENWIDTH-switchW-10, (50 - switchH)/2, switchW,switchH);
        ...
        headerImg = [UIImage imageNamed:@"forcePassword"];
        imgW = kSCREENWIDTH;
        imgH = kHEAD_VIEW_H;
        UIImageView *hBagImgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, imgW, imgH)];
        hBagImgView.image = headerImg;
        [cell.contentView addSubview:hBagImgView];
        
        UIImage *centerImg = [UIImage imageNamed:@"passwordCenter"];
        CGFloat centerImgW = kSCREENWIDTH*0.7;
        CGFloat centerImgH = centerImg.size.height/centerImg.size.width*centerImgW;
        
        UIImageView *centerImgView = [[UIImageView alloc] initWithFrame:CGRectMake((CGRectGetWidth(hBagImgView.frame)-centerImgW)/2, (CGRectGetHeight(hBagImgView.frame)-30 - centerImgH)/2+10, centerImgW, centerImgH)];
        centerImgView.image = centerImg;
        [hBagImgView addSubview:centerImgView];
        
        UILabel *tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(hBagImgView.frame)- 30, CGRectGetWidth(hBagImgView.frame), 20)];
        tipLabel.textColor = [UIColor whiteColor];
        tipLabel.font = kTEXT_FONT;
        tipLabel.textAlignment = NSTextAlignmentCenter;
        tipLabel.text = @"为了您的信息安全请立即设定新的密码!";
        [hBagImgView addSubview:tipLabel];
...
- (UIView*)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    ...
    if (1 == section) {
        ...
        [bottomBtn.layer setCornerRadius:2];
